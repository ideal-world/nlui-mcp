...
function removeDuplicateExamples(schemas: Record<string, any>): Record<string, any> {
  ...
  function removeDuplicateExamplesFromObject(obj: any, isTopLevelSchema = false, schemaKey = '', path = ''): void {
    ...
    if (obj.anyOf) {
      obj.anyOf.forEach((item: any, index: number) => removeDuplicateExamplesFromObject(item, false, schemaKey, `${path}.anyOf[${index}]`));
    }

    if (obj.allOf) {
      obj.allOf.forEach((item: any, index: number) => removeDuplicateExamplesFromObject(item, false, schemaKey, `${path}.allOf[${index}]`));
    }

    if (obj.oneOf) {
      obj.oneOf.forEach((item: any, index: number) => removeDuplicateExamplesFromObject(item, false, schemaKey, `${path}.oneOf[${index}]`));
    }
  ...
  function extractExportedInterfaces(filePaths: string[]): string[] {
    ...
    for (const filePath of filePaths) {
      try {
        ...
        if (interfaceMatches) {
          const interfaceNames = interfaceMatches.map((match) => match.replace(/export\s+interface\s+/, ''));
...
// Main execution
const isMainModule = import.meta.url.startsWith('file:') && import.meta.url.includes('generate-schemas.ts');
