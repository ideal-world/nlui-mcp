...
<div class="relative">
  <button class="btn btn-text btn-xs" onclick={() => (isDropdownOpen = !isDropdownOpen)} aria-label="Switch theme" title="Switch theme">
    <span class="text-sm">
  ...
  {#if isDropdownOpen}
    <div class="card card-sm absolute top-full right-0 z-[100] mt-1 max-h-64 w-48 overflow-y-auto py-1">
      {#each themes as theme}
        <button class="text-base-content hover:bg-base-200 flex w-full items-center gap-2 px-3 py-1.5 text-xs {currentTheme === theme.id ? 'bg-primary/10 text-primary' : ''}" onclick={() => handleThemeChange(theme.id)}>
...
{#if isDropdownOpen}
  <div class="fixed inset-0 z-[99]" onclick={() => (isDropdownOpen = false)} aria-hidden="true"></div>
{/if}
