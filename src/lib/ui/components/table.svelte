<script lang="ts">
	...
	import {
		getBaseClasses,
		getIconClass,
		getSmallerSizeClassSuffix,
		handleAction
	} from '../common/base.utils.js';
	...
	let filteredRows = $derived(
		searchQuery.trim()
			? tableProps.rows.filter((row) =>
					tableProps.columns.some((column) =>
						getCellData(row, column.key).toLowerCase().includes(searchQuery.toLowerCase())
					)
				)
			: tableProps.rows
	);
	...
	let totalPages = $derived(
		tableProps.pageSize ? Math.ceil(filteredRows.length / tableProps.pageSize) : 1
	);

	let pageRows = $derived(
		tableProps.pageSize
			? filteredRows.slice(
					(currentPage - 1) * tableProps.pageSize,
					currentPage * tableProps.pageSize
				)
			: filteredRows
	);
		...
		<table class={getBaseClasses('table', tableProps.size)}>
			...
			<tbody>
				{#if pageRows.length === 0}
					<tr>
						<td
							colspan={tableProps.columns.length + (tableProps.actions ? 1 : 0)}
						...
						<tr>
							{#each tableProps.columns as column}
								<td>
									{#if column.tagStyle}
										<span
											class="badge badge-soft badge-success text-{getSmallerSizeClassSuffix(
												tableProps.size
											)}"
										>
							...
							{#if tableProps.actions}
								<td>
									{#each tableProps.actions as action}
										{#if action.type}
											<button
												class="btn btn-circle btn-text btn-{getSmallerSizeClassSuffix(
													tableProps.size
												)}"
												aria-label={action.label || ''}
												onclick={() => handleAction('table', action, { id: row.id })}
											>
			...
			{#if totalPages > 1}
				<nav class="join">
					...
					{#each pageNumbers as pageNum}
						<button
							type="button"
							class="btn btn-soft join-item btn-square {currentPage === pageNum
								? 'btn-primary'
								: ''}"
							onclick={() => goToPage(pageNum)}
							aria-label={m.table_page({ page: pageNum })}
						>
							{pageNum}
						</button>
					{/each}
