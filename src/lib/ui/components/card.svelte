...
<div class={getBaseClasses('card', cardProps.size)}>
  {#if cardProps.image}
    <figure>
      <img src={cardProps.image} alt="" class="transition-transform duration-500 group-hover:scale-110" />
    </figure>
  {/if}
    ...
    {#if cardProps.primaryAction || cardProps.secondaryAction}
      <div class="card-actions">
        {#if cardProps.primaryAction}
          <button class="btn btn-primary" onclick={async () => await handleAction('card', cardProps.primaryAction!)}>
            {cardProps.primaryAction.label}
          </button>
        {/if}
        {#if cardProps.secondaryAction}
          <button class="btn btn-secondary" onclick={async () => await handleAction('card', cardProps.secondaryAction!)}>
