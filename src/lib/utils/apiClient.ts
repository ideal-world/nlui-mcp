...
export class ApiClient {
  ...
  constructor(baseUrl: string = '', defaultHeaders: Record<string, string> = {}, defaultTimeout: number = 30000) {
   ...
   * Send HTTP request
   */
  async sendRequest<T>(endpoint: string, config: ApiRequestConfig = {}): Promise<ApiResponse<T>> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw ErrorHandler.createApiError(errorData.message || `HTTP ${response.status}: ${response.statusText}`, endpoint, response.status);
      }
  ...
  async requestWithRetry<T>(endpoint: string, config: ApiRequestConfig = {}): Promise<T> {
    ...
    return retry(() => this.sendRequest<T>(endpoint, requestConfig).then((res) => res.data), retries, retryDelay);
  }
